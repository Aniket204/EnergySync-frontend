...
const BigCard = ({ title, caption, Icon, active, level }: BigCardProps) => {
  ...
  return (
    <Card className={`relative overflow-hidden shadow-sm rounded-sm p-4 mx-2 flex-1 flex flex-col items-center justify-center ${active ? 'shadow-md' : 'shadow-sm opacity-50'}`}>
      {active && (
        <div
          className="absolute top-0 left-0 opacity-75 w-[300%] h-full bg-gradient-to-r from-cyan-300/30 via-cyan-400/50 to-cyan-300/30 dark:from-cyan-600/30 dark:via-cyan-500/50 dark:to-cyan-600/30 animate-slide"
          style={{
